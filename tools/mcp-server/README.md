...
| Tool | What it does |
|---|---|
| `list_projects`     | Returns every `apps/*` and `libs/*` with its CLAUDE.md summary. |
...
| `run`               | `bazel run <target>` — restricted to an allowlist parsed from each project's CLAUDE.md. |
