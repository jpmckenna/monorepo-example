...
2. Read that project's local `AGENTS.md` — it lists the commands, conventions,
   and the explicit set of domains it's allowed to depend on.
3. Use only the Bazel targets the local AGENTS.md lists. The MCP server in
...
Per-language specifics live in each project's local `AGENTS.md`.
